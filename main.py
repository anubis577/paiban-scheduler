    ...
    def _check_and_update_border_style_core(self):
        ...
        if not violation_msg and hasattr(main_window, 'template_slots'):
            ...
            for p in selected_people:
                persons_dict[p['name']] = {'name': p['name'], 'level': p['level'], 'score': p.get('score', 0)}
...
class MainWindow(QMainWindow):
    ...
    def load_templates(self):
        ...
        for i, tpl in enumerate(templates):
            tpl = dict(tpl)
            self.templates_table.setItem(i, 0, QTableWidgetItem(tpl['id']))
    ...
    def add_template(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            success, msg = self.db.add_template(data['id'], data['name'], data.get('description', ''), data.get('time_start', '08:40'), data.get('time_end', '10:30'))
            if success:
                # 设置默认位置
    ...
    def edit_template(self):
        ...
        if template:
            ...
            if dialog.exec_() == QDialog.Accepted:
                ...
                old_id = template_id
                new_id = data.get('id')
                self.db.update_template(old_id, data.get('name'), data.get('description'), new_id=new_id)
                self.load_templates()
    ...
    def on_template_available_clicked(self, row, col):
        ...
        if col == 3:
            ...
            if dialog.exec_() == QDialog.Accepted:
                data = dialog.get_data()
                self.db.update_template(template_id, data.get('name'), data.get('description'))
                self.load_templates()
...
# 时段配置标题
    ...
    def edit_template_time_slots(self, template_id, template):
        ...
        for row in range(self.templates_table.rowCount()):
            if self.templates_table.item(row, 0).text() == template_id:
    ...
    def create_schedule_tab(self):
        ...
        for seat_data in all_seats:
            seat = Seat(seat_data['app_name'], bool(seat_data.get('available', 1)), seat_data.get('persons_count', 3), seat_data.get('required_score', 5))
...
key=app_sort_key
    ...
    def update_seat_result_widgets(self):
        ...
        for i, widget in enumerate(self.seat_result_widgets):
            if i < len(seats):
                new_seat = Seat(seats[i]['app_name'], bool(seats[i].get('available', 1)), seats[i].get('persons_count', 3), seats[i].get('required_score', 5))
    ...
    def _run_schedule_original(self):
        ...
        try:
            ...
            for seat_id, names in user_selections.items():
                for pos_idx, name in enumerate(names):
                    ...
                    if p:
                        ...
                        all_person_objs.append(Person(p['id'], p['name'], p['level'], p['score'], True, score_mod, seat_history, total_hours, True, position=pos_idx, last_seat_app_name=last_seat_app_name))
            ...
            for p in remaining_persons:
                ...
                all_person_objs.append(Person(p_dict['id'], p_dict['name'], p_dict['level'], p_dict['score'], True, score_mod, seat_history, total_hours, False, last_seat_app_name=last_seat_app_name))
            
            seats = [
                Seat(s['app_name'], bool(s.get('available', 1)), s.get('persons_count', 3), s.get('required_score', 5))
                for s in available_seats
            ]
    ...
    def export_to_excel(self):
        try:
            ...
            if platform.system() == 'Windows':
                # Windows默认路径
                last_dir = getattr(self, '_last_export_dir', os.path.expanduser('~'))
                default_dir = last_dir if os.path.exists(last_dir) else 'C:\\'
            else:
                # Mac默认路径
                last_dir = getattr(self, '_last_export_dir', os.path.expanduser('~'))
                default_dir = last_dir if os.path.exists(last_dir) else os.path.expanduser('~')
            
            file_path, _ = QFileDialog.getSaveFileName(self, "导出排班表", default_dir, "Excel文件 (*.xlsx)")
            if not file_path:
                return
            if not file_path.endswith('.xlsx'):
                file_path += '.xlsx'
            
            # 跨平台处理：MAC和Windows区分（放在前面确保_default_dir已定义）
            import platform
            import os
            if not hasattr(self, '_last_export_dir'):
            ...
            if duty_summary:
                ...
                for app_name, persons in seat_map.items():
                    ...
                    for p in persons:
                        ...
                        if duty_count > 0:
                            self.db.increment_duty_count(person['id'], app_name, duty_count)
                            self.db.update_person_last_seat(person['id'], app_name)
